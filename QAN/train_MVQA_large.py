...
class FGCDataset(Dataset):
    def __init__(self, split, datapath_list, bwd=False):
        with open('Word2Vec_Chinese_W2I.pkl', 'rb') as handle:
            w2v_w2i_table = pickle.load(handle)
        with open('GloVe_Chinese_W2I.pkl', 'rb') as handle:
            glove_w2i_table = pickle.load(handle)

        assert split in ('train', 'dev', 'test', 'train_ranker')
        self.split = split
        ...
        for datapath in datapath_list:
            ...
            for PQA in input_data:
                ...
                passage_jieba = list(jieba.cut(raw_passage, cut_all=False, HMM=True))
                passage = tokenizer.tokenize(raw_passage)
                passage_no_unk = tokenize_no_unk(tokenizer, raw_passage)
                passage_jieba = bert_glove_match(passage_jieba, passage_no_unk)
                # pdb.set_trace()
                ...
                for QA in PQA['QUESTIONS']:
                    ...
                    if len(raw_question) == 0:
                        raw_question = '无问题'
                        continue
                    question_jieba = list(jieba.cut(raw_question, cut_all=False, HMM=True))
                    question = tokenizer.tokenize(raw_question)
                    question_no_unk = tokenize_no_unk(tokenizer, raw_question)
                    question_jieba = bert_glove_match(question_jieba, question_no_unk)
                    ...
                    if answer_start >= 0:
                        ...
                        processed_QA['question_jieba'] = question_jieba
                    ...
                    for QA in QAs:
                        ...
                        qtext_jieba_tokens = QA['question_jieba']
                        qtext_no_unk_tokens = QA['question_no_unk']
                        dtext_tokens = passage
                        dtext_jieba_tokens = passage_jieba
                        ...
                        qtext_jieba_tokens.insert(0, '<UNK>')
                        qtext_jieba_tokens.append('<UNK>')
                        ...
                        if diff > 0:
                            ...
                            if self.split == 'train_ranker':
                                if self.bwd:
                                    ...
                                    dtext_jieba_tokens = dtext_jieba_tokens[diff:]
                                else:
                                    if answer_end > 510:
                                        rank_label[0] = 0
                                    else:
                        ...
                        inupt_glove_ids = covert_seq2id(input_jieba_tokens, glove_w2i_table)
                        input_w2v_ids = covert_seq2id(input_jieba_tokens, w2v_w2i_table)
                        inupt_glove_ids = torch.LongTensor(inupt_glove_ids)
                        input_w2v_ids = torch.LongTensor(input_w2v_ids)
                        ...
                        input_item['inupt_glove_ids'] = inupt_glove_ids
                        input_item['input_w2v_ids'] = input_w2v_ids
                        input_item['rank_truth'] = rank_label
    ...
    def __getitem__(self, i):
        ...
        inupt_glove_ids = self.datas[i]['inupt_glove_ids']
        input_w2v_ids = self.datas[i]['input_w2v_ids']
        rank_truth = self.datas[i]['rank_truth']

        if self.split == 'train':   # Answer for training
            return input_ids, attention_mask, token_type_ids, start_positions, end_positions, inupt_glove_ids, input_w2v_ids, rank_truth
        elif self.split == 'train_ranker':   # Answer for training
            return input_ids, attention_mask, token_type_ids, start_positions, end_positions, inupt_glove_ids, input_w2v_ids, rank_truth
        else:   # Mask out question tokens for inference
            return input_ids, attention_mask, token_type_ids, margin_mask, input_no_unk_tokens, answer, inupt_glove_ids, input_w2v_ids, rank_truth
...
def get_FGC_dataloader(split, bwd=False, batch_size=1, num_workers=0, datapath_list=None):
    ...
    def train_collate_fn(batch):
        input_ids, attention_mask, token_type_ids, start_positions, end_positions, inupt_glove_ids, input_w2v_ids, rank_truth = zip(*batch)
        input_ids = pad_sequence(input_ids, batch_first=True)
        inupt_glove_ids = pad_sequence(inupt_glove_ids, batch_first=True, padding_value=0)
        input_w2v_ids = pad_sequence(input_w2v_ids, batch_first=True, padding_value=0)
        ...
        rank_truth = torch.stack(rank_truth)
        return input_ids, attention_mask, token_type_ids, start_positions, end_positions, inupt_glove_ids, input_w2v_ids, rank_truth
    
    def test_collate_fn(batch):
        input_ids, attention_mask, token_type_ids, margin_mask, input_tokens_no_unk, answers, inupt_glove_ids, input_w2v_ids, rank_truth = zip(*batch)
        input_ids = pad_sequence(input_ids, batch_first=True)
        inupt_glove_ids = pad_sequence(inupt_glove_ids, batch_first=True, padding_value=0)
        input_w2v_ids = pad_sequence(input_w2v_ids, batch_first=True, padding_value=0)
        attention_mask = pad_sequence(attention_mask, batch_first=True)
        ...
        rank_truth = torch.stack(rank_truth)
        return input_ids, attention_mask, token_type_ids, margin_mask, input_tokens_no_unk, answers, inupt_glove_ids, input_w2v_ids, rank_truth
...
def validate_dataset(model, ranker, fwd_dataloader, bwd_dataloader, split, topk=1, datapath=None):
    ...
    for fwd_batch, bwd_batch in zip(fwd_dataloader, bwd_dataloader):
        ##### Forward padding ####
        input_ids, attention_mask, token_type_ids, margin_mask, fwd_input_tokens_no_unks, answers, inupt_glove_ids, inupt_w2v_ids = fwd_batch
        ...
        #### Backward padding ####
        input_ids, attention_mask, token_type_ids, margin_mask, bwd_input_tokens_no_unks, answers, inupt_glove_ids, inupt_w2v_ids = bwd_batch
        input_ids = input_ids.cuda(device=device)
        attention_mask = attention_mask.cuda(device=device)
        token_type_ids = token_type_ids.cuda(device=device)
        margin_mask = margin_mask.cuda(device=device)
        ...
        for i, answer in enumerate(answers):
            ...
            for n in range(topk*5):
                ...
                if fwd_out[i][0].item() < bwd_out[i][0].item():
                    start_prob += -1e10
                ...
                if bwd_out[i][0].item() <= fwd_out[i][0].item():
                    start_prob += -1e10
...
def validate(model, ranker, fwd_dataloader_d, bwd_dataloader_d, fwd_dataloader_t, bwd_dataloader_t, topk=1):
    ''' Perform a verbose validation on both dev/test split. '''

    # Valid set
    val_em, val_f1, val_count = validate_dataset(model, ranker, fwd_dataloader_d, bwd_dataloader_d, 'dev', topk, 'movieqa_dev.json')
    val_avg_em = 100 * val_em / val_count
    val_avg_f1 = 100 * val_f1 / val_count

    # Test set
    test_em, test_f1, test_count = validate_dataset(model, ranker, fwd_dataloader_t, bwd_dataloader_t, 'test', topk, 'movieqa_test.json')
...
if __name__ == '__main__':
    ...
    glove_table = np.load('GloVe_Chinese_I2V.npy')
    word2vec_table = np.load('Word2Vec_Chinese_I2V.npy')

    # model = BertForQuestionAnswering.from_pretrained('bert-base-chinese')
    model = BertForQuestionAnswering.from_pretrained('hfl/chinese-roberta-wwm-ext-large')
    ...
    ranker = BertForSequenceClassification.from_pretrained('hfl/chinese-roberta-wwm-ext-large')
    ...
    optimizer_rank = AdamW(ranker.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-2, amsgrad=False)
    ...
    fwd_dataloader_rank = get_FGC_dataloader('train_ranker', bwd=False, batch_size=batch_size, num_workers=8, datapath_list=datapath_list_train)
    bwd_dataloader_rank = get_FGC_dataloader('train_ranker', bwd=True, batch_size=batch_size, num_workers=8, datapath_list=datapath_list_train)
    ...
    fwd_dataloader_d_dr = get_FGC_dataloader('dev', bwd=False, batch_size=16, num_workers=16, datapath_list=['DRCD_dev.json'])
    bwd_dataloader_d_dr = get_FGC_dataloader('dev', bwd=True, batch_size=16, num_workers=16, datapath_list=['DRCD_dev.json'])
    fwd_dataloader_t_dr = get_FGC_dataloader('test', bwd=False, batch_size=16, num_workers=16, datapath_list=['DRCD_test.json'])
    bwd_dataloader_t_dr = get_FGC_dataloader('test', bwd=True, batch_size=16, num_workers=16, datapath_list=['DRCD_test.json'])

    fwd_dataloader_d_kg = get_FGC_dataloader('dev', bwd=False, batch_size=16, num_workers=16, datapath_list=['Kaggle_dev.json'])
    bwd_dataloader_d_kg = get_FGC_dataloader('dev', bwd=True, batch_size=16, num_workers=16, datapath_list=['Kaggle_dev.json'])
    fwd_dataloader_t_kg = get_FGC_dataloader('test', bwd=False, batch_size=16, num_workers=16, datapath_list=['Kaggle_test.json'])
    bwd_dataloader_t_kg = get_FGC_dataloader('test', bwd=True, batch_size=16, num_workers=16, datapath_list=['Kaggle_test.json'])

    fwd_dataloader_d_fg = get_FGC_dataloader('dev', bwd=False, batch_size=16, num_workers=16, datapath_list=['FGC_release_all_dev.json'])
    bwd_dataloader_d_fg = get_FGC_dataloader('dev', bwd=True, batch_size=16, num_workers=16, datapath_list=['FGC_release_all_dev.json'])
    fwd_dataloader_t_fg = get_FGC_dataloader('test', bwd=False, batch_size=16, num_workers=16, datapath_list=['FGC_release_all_test.json'])
    bwd_dataloader_t_fg = get_FGC_dataloader('test', bwd=True, batch_size=16, num_workers=16, datapath_list=['FGC_release_all_test.json'])

    fwd_dataloader_d_le = get_FGC_dataloader('dev', bwd=False, batch_size=16, num_workers=16, datapath_list=['Lee_dev.json'])
    bwd_dataloader_d_le = get_FGC_dataloader('dev', bwd=True, batch_size=16, num_workers=16, datapath_list=['Lee_dev.json'])
    fwd_dataloader_t_le = get_FGC_dataloader('test', bwd=False, batch_size=16, num_workers=16, datapath_list=['Lee_test.json'])
    bwd_dataloader_t_le = get_FGC_dataloader('test', bwd=True, batch_size=16, num_workers=16, datapath_list=['Lee_test.json'])


    for itr in tqdm(range(10)):
        for fwd_batch, bwd_batch in zip(fwd_dataloader_rank, fwd_dataloader_rank):

            ranker.train()

            input_ids, attention_mask, token_type_ids, _, _, _, _, rank_truth = fwd_batch
            input_ids = input_ids.cuda(device=device)
            attention_mask = attention_mask.cuda(device=device)
            token_type_ids = token_type_ids.cuda(device=device)
            rank_truth = rank_truth.cuda(device=device)
            ...
            input_ids, attention_mask, token_type_ids, _, _, _, _, rank_truth = bwd_batch
            input_ids = input_ids.cuda(device=device)
            attention_mask = attention_mask.cuda(device=device)
            token_type_ids = token_type_ids.cuda(device=device)
            rank_truth = rank_truth.cuda(device=device)
        ...
        val_ranker_em = validate_ranker(ranker, fwd_dataloader_d_fg, bwd_dataloader_d_fg)

        if best_ranker_em <= val_ranker_em:
            torch.save(ranker.state_dict(), 'bestRoBERTaRanker_full_dev_em.pth')
    ...
    for itr in tqdm(range(20)):
        ...
        for batch in dataloader:
            input_ids, attention_mask, token_type_ids, start_positions, end_positions, inupt_glove_ids, inupt_w2v_ids, rank_truth, input_ids_fwd = batch
            ...
            loss, _, _ = model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, \
        ...
        _ = validate(model, ranker, fwd_dataloader_d, bwd_dataloader_d, fwd_dataloader_t, bwd_dataloader_t, topk=1)
        _ = validate(model, ranker, fwd_dataloader_d_dr, bwd_dataloader_d_dr, fwd_dataloader_t_dr, bwd_dataloader_t_dr, topk=1)
        _ = validate(model, ranker, fwd_dataloader_d_kg, bwd_dataloader_d_kg, fwd_dataloader_t_kg, bwd_dataloader_t_kg, topk=1)
        val_f1 = validate(model, ranker, fwd_dataloader_d_fg, bwd_dataloader_d_fg, fwd_dataloader_t_fg, bwd_dataloader_t_fg, topk=1)
        _ = validate(model, ranker, fwd_dataloader_d_le, bwd_dataloader_d_le, fwd_dataloader_t_le, bwd_dataloader_t_le, topk=1)
        # em_dev_current, f1_dev_current, em_test_current, f1_test_current = validate(model)

        if f1_best < val_f1:
            # torch.save(model.state_dict(), 'bestBERTFGCWiki_full_dev_f1.pth')
            torch.save(model.state_dict(), 'bestRoBERTaLargeMVQA_full_dev_f1_mvqa0.pth')
            # torch.save(model.state_dict(), 'bestZhRoBERTaMVQA_full_dev_f1_fgc0.pth')
            # torch.save(model.state_dict(), 'bestZhLRoBERTaMVQA_full_dev_f1_fgc0.pth')

            torch.save(ranker.state_dict(), 'bestRankerLargeMVQA_full_dev_f1_fgc0.pth')
