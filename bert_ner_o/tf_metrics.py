...
def safe_div(numerator, denominator):
    ...
    denominator_is_zero = tf.math.equal(denominator, zeros)
...
def pr_re_fbeta(cm, pos_indices, beta=1):
    ...
    diag_sum = tf.math.reduce_sum(tf.linalg.diag_part(cm * cm_mask))

    cm_mask = np.ones([num_classes, num_classes])
    cm_mask[:, neg_indices] = 0
    tot_pred = tf.math.reduce_sum(cm * cm_mask)

    cm_mask = np.ones([num_classes, num_classes])
    cm_mask[neg_indices, :] = 0
    tot_gold = tf.math.reduce_sum(cm * cm_mask)
...
def metrics_from_confusion_matrix(cm, pos_indices=None, average='micro',
    ...
    if average == 'micro':
        ...
        for idx in pos_indices:
            ...
            cm_mask[idx, :] = 1
            n_golds.append(tf.compat.v1.to_float(tf.math.reduce_sum(cm * cm_mask)))

        if average == 'macro':
            pr = tf.math.reduce_mean(precisions)
            re = tf.math.reduce_mean(recalls)
            fbeta = tf.math.reduce_mean(fbetas)
            return pr, re, fbeta
        if average == 'weighted':
            n_gold = tf.math.reduce_sum(n_golds)
